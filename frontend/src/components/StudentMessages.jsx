...
import { MessageSquare, Send, Image as ImageIcon, Info } from 'lucide-react';
...
const StudentMessages = () => {
    ...
    // Fetch from LocalStorage (set during Login)
    ...
    const handleSend = async (e) => {
        ...
        const payload = {
            ...
            is_anonymous: true // ALWAYS Anonymous for peers
        };
    ...
    return (
        <div className="p-6 h-[calc(100vh-100px)] flex flex-col max-w-4xl mx-auto">
            <header className="flex justify-between items-center mb-6 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-3">
                        <MessageSquare className="w-6 h-6 text-indigo-600" /> Section {section} Chat
                    </h1>
                </div>
                <div className="flex items-center gap-2 text-xs text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-full font-medium">
                    <Info className="w-3 h-3" />
                    <span>Your identity is hidden from classmates</span>
                </div>
            </header>
            ...
            <div className="flex-1 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
                    {messages.length === 0 ? (
                        <div className="text-center text-gray-400 mt-10">No messages in Section {section} yet. Ask a doubt!</div>
                    ) : (
                        messages.map((msg) => (
                            <div key={msg.id} className={`flex ${msg.is_me ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[70%] rounded-2xl p-4 ${msg.is_me ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white border border-gray-200 rounded-tl-none'}`}>
                                    {!msg.is_me && (
                                        <p className={`text-xs font-bold mb-1 ${msg.role === 'teacher' ? 'text-violet-600' : 'text-gray-500'}`}>
                                            {msg.sender_name}
                                        </p>
                                    )}
                                    {msg.image_url && (
                ...
                <form onSubmit={handleSend} className="p-4 bg-white border-t border-gray-100 flex gap-4 items-end">
                    <div className="flex-1 bg-gray-50 rounded-xl border border-gray-200 flex items-center px-4 py-2">
                        ...
                        />
                        <label className="cursor-pointer text-gray-400 hover:text-indigo-600 transition-colors p-2">
                    ...
                    <button
                        ...
                        className="p-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-200"
                    >
